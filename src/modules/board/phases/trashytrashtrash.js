(function webpackUniversalModuleDefinition(root, factory) {
    if (typeof exports === "object" && typeof module === "object") module.exports = factory(require("lodash"), require("react"));
    else if (typeof define === "function" && define.amd) define(["_", "react"], factory);
    ...
    return function(modules) {
        ...
        function expandPath(path) {
            var expanded = [],
                len = path.length,
                coord0, coord1, interpolated, interpolatedLen, i, j;
            if (len < 2) {
                return expanded
            }
            for (i = 0; i < len - 1; ++i) {
        ...
        function smoothenPath(grid, path) {
            var len = path.length,
                ...
                x1 = path[len - 1][0],
                y1 = path[len - 1][1],
                sx, sy, ex, ey, newPath, i, j, coord, line, testCoord, blocked;
            sx = x0;
            sy = y0;
            newPath = [
                [sx, sy]
        ...
        JumpPointFinderBase.prototype._identifySuccessors = function(node) {
            var grid = this.grid,
                ...
                endX = this.endNode.x,
                endY = this.endNode.y,
                neighbors, neighbor, jumpPoint, i, l, x = node.x,
                y = node.y,
                jx, jy, dx, dy, d, ng, jumpNode, abs = Math.abs,
                max = Math.max;
        ...
        BiAStarFinder.prototype.findPath = function(startX, startY, endX, endY, grid) {
            var cmp = function(nodeA, nodeB) {
                ...
                abs = Math.abs,
                SQRT2 = Math.SQRT2,
                node, neighbors, neighbor, i, l, x, y, ng, BY_START = 1,
                BY_END = 2;
            startNode.g = 0;
            startNode.f = 0;
        ...
        BiBreadthFirstFinder.prototype.findPath = function(startX, startY, endX, endY, grid) {
            var startNode = grid.getNodeAt(startX, startY),
                ...
                startOpenList = [],
                endOpenList = [],
                neighbors, neighbor, node, diagonalMovement = this.diagonalMovement,
                BY_START = 0,
                BY_END = 1,
                i, l;
            startOpenList.push(startNode);
